...
def test_blended_predictions_correct_dimensions():
    ...
    p0 = [0.01, 0.09, 0.17, 0.31, 0.39, 0.54, 0.59, 0.69, 0.77, 0.92, 0.99]
    p1 = [0.00, 0.12, 0.19, 0.32, 0.42, 0.51, 0.61, 0.68, 0.81, 0.91, 1.02]

    bst = BlendingSwarmTransformer(metric=mean_absolute_error, maximize=False)
    p_blended = bst.fit_transform([p0, p1], target)
...
def test_fixed_random_state_deterministic_minimize():
    ...
    bst = BlendingSwarmTransformer(metric=mean_absolute_error, maximize=False)
    p_blended_0 = bst.fit_transform([p0, p1], target, random_state=1)
    p_blended_1 = bst.fit_transform([p0, p1], target, random_state=1)
...
def test_blended_results_minimize():
    ...
    bst = BlendingSwarmTransformer(metric=mean_absolute_error, maximize=False)
    p_blended = bst.fit_transform([p0, p1], target, random_state=1)

    mse_0 = mean_absolute_error(target, p0)
    mse_1 = mean_absolute_error(target, p1)
...
def test_blended_results_maximize():
    ...
    auc_0 = roc_auc_score(target, p0)
    auc_1 = roc_auc_score(target, p1)
